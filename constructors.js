...
//title (a string) and rating (a number). Use these parameters to create a new object and add it to the savedPosts array. Make sure to name the properties the same as described previously (id, title, rating).
...
User.prototype.removeSavedPost = function(id) {
  let findPost = this.savedPosts.findIndex(post => post == id);
  let removePost = this.savedPosts.splice(findPost, 1);
  return removePost;
}
...
User.prototype.changePostRating = function(id, newRating) {
  let found = this.savedPosts.find(post => post.id == id);
